...
import "./interfaces/IWemixPunks.sol";
import "./libraries/Signature.sol";

contract WemixPunks is Ownable, ERC721("WemixPunks", "PUNKS"), IERC2981, IWemixPunks {
    ...
    constructor(address _feeReceiver, uint256 _fee) {
        ...
        _HASHED_NAME = keccak256(bytes("WemixPunks"));
        ...
        __baseURI = "https://api.wemixpunks.com/metadata/";
        contractURI = "https://api.wemixpunks.com/wemixpunks";
    }
    ...
    function permit(
        ...
        require(block.timestamp <= deadline, "WemixPunks: Expired deadline");
        ...
        require(spender != owner, "WemixPunks: Invalid spender");

        if (Address.isContract(owner)) {
            require(
                IERC1271(owner).isValidSignature(digest, abi.encodePacked(r, s, v)) == 0x1626ba7e,
                "WemixPunks: Unauthorized"
            );
        } else {
            address recoveredAddress = Signature.recover(digest, v, r, s);
            require(recoveredAddress == owner, "WemixPunks: Unauthorized");
        }
    ...
    function setMinter(address target, bool _isMinter) external onlyOwner {
        require(isMinter[target] != _isMinter, "WemixPunks: Permission not changed");
    ...
    function _setRoyaltyInfo(address _receiver, uint256 _fee) internal {
        require(_fee < 10000, "WemixPunks: Invalid Fee");
    ...
    function mint(address to) external returns (uint256 id) {
        require(isMinter[msg.sender], "WemixPunks: Forbidden");
        id = totalSupply;
        _mint(to, id);
        totalSupply += 1;
    }
